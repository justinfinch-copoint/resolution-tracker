...
import { mergeUserSummary } from '../memory/long-term/user-summary';
...
export function createCoachAgentTools(userId: string) {
  return {
    ...
    resumeGoal: tool({
      description: 'Resume a paused goal. Checks active goal limit (max 5).',
      inputSchema: z.object({
...
export type CoachTools = ReturnType<typeof createCoachAgentTools>;
